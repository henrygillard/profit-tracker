 ...
 *
 * @returns {object} - { revenueNet, cogsTotal, feesTotal, shippingCost, netProfit, cogsKnown, feeSource }
 *                     cogsTotal and netProfit are null when cogsKnown is false
 *                     feeSource is 'pending' | 'estimated' (never 'verified' — syncPayouts sets that)
 */
function calculateOrderProfit(order) {
  ...
  return {
    ...
    netProfit,
    cogsKnown,
    feeSource,  // 'pending' | 'estimated' (never 'verified' — syncPayouts sets that)
  };
}

module.exports = { calculateOrderProfit, getCOGSAtTime, getThirdPartyFeeRate, THIRD_PARTY_FEE_RATES, determineFeeSourceFromOrder };
